...
import com.largehat.common.cache.IL2Cache;
...
public class RedisConfigurationFactory {
    ...
    private static final String DEFAULT_CLASSPATH_CONFIGURATION_FILE = "cache.properties";
    ...
    public static RedisConfiguration parseConfiguration() throws Exception {
        ...
        if (url == null) {
        	url = IL2Cache.class.getResource(DEFAULT_CLASSPATH_CONFIGURATION_FILE);
        }
        if (url != null) {
            LOG.debug("Configuring redis from im.properties found in the classpath: " + url);
        } else {
            LOG.warn("No configuration found. Configuring redis from im.properties " + " found in the classpath: {}", url);
        }
